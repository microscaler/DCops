...
mod tests {
    ...
    async fn test_reconcile_ip_pool_status_update() {
        ...
        // Now that we're using ipnet for proper IP network checking, the filter will correctly
        // identify that "192.168.1.2/24" is within the prefix "192.168.1.0/24"
        ...
        // With 1 allocated IP (now properly found by ipnet-based filter) and 1 available IP, total should be 2
        assert_eq!(status.total_ips, 2, "Total IPs should be 2 (1 allocated + 1 available)");
